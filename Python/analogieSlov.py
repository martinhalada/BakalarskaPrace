...
def analogie(slova, zadane_slovo):
    nejblizsi_vzdalenost = [0]*50
    nejblizsi_slova = [""]*50
    vector = [0]*100

    #zjistit, jestli existuje slovo
    _slova = zadane_slovo.split(" ")
    print(_slova)
    if _slova[0] not in slova:
        print("Zadané slovo neexistuje ve slovníku, zkuste zadat jiné slovo.")
        return -1
    if _slova[1] not in slova:
        print("Zadané slovo neexistuje ve slovníku, zkuste zadat jiné slovo.")
        return -1
    if _slova[2] not in slova:
        print("Zadané slovo neexistuje ve slovníku, zkuste zadat jiné slovo.")
        return -1

    #výpočet vzdálenosti k ostatním slovům
    for i in range(100):
        vector[i] = (slova[_slova[1]][i]-slova[_slova[0]][i]) + slova[_slova[2]][i]
    vzdalenost = 0
    for i in range(100):
...
def nacti_testovaci_slova():
    slovnik = {}
    with open("analogie_test_data.txt", "r", encoding="utf-8") as in_file:
        for slovo in in_file:
            slova = slovo.strip().split(" ")
            key = slova[0] + " " + slova[1] + " " + slova[2]
            slovnik[key]=slova[3]
    return slovnik
...
def zjistit_uspesnost(slova, test):
    ...
    for key in enumerate(test):
        slovo = key[1]
        nejblizsi_slova = analogie(slova, slovo)
...
if __name__ == "__main__":
    ...
    while True:
        val = input("Zadej 3 slova odělené mezerou (ukonči slovem: exit): ")
        val = val.lower()
        if "exit" in val:
            break
        else:
